...
struct entry {
    ...
    bool operator>(const entry& o) const {
        if (g_value + h_value != o.g_value + o.h_value) return g_value + h_value > o.g_value + o.h_value;
        return g_value > o.g_value;
    }
};
...
struct node {
    ...
    node(const position& pos, ll v) : pos(pos), value(v), adj(vector<edge>()), used(false) { }
};
...
void find_shortest_path(const position& from, const position& to, function<ll(const position&)> heuristic) {
    ...
    explore_node(from);
    pq.push({from, 0, h_function(position {0, 0})});
    ...
    while (!pq.empty()) {
        ...
        for (const edge& p : node_.adj) {
            ...
            if (!get_node(p.pos)->used) {
                // cout << "inserting " << p.pos.y << " " << p.pos.x << " g:" << p.weight + e.g_value << ", h:" << h_function(p.pos) << "\n";
...
void align_with_heuristic() {
    ...
    int k = max(2, (int) log(m));

    suffix_non_matches_along_x = compute_suffix_non_matches(v, w, k);
    suffix_non_matches_along_y = compute_suffix_non_matches(w, v, k);
